...
> 状态时间锚点：本地分支 `main`，HEAD = `09ccae51b`（批次后修复：GUI 块头吸顶）。
> **批次 1–4 全部完成**，工作区干净；批次后另修复 2 个用户反馈问题（见 §10）。
...
## 10. 批次后问题修复（2026-09-01，用户反馈驱动）

统一验证前用户试用时反馈两个问题，均已修复并本地提交。

### 10.1 原始视图输入不可见（commit `c4997c6ba`，重要 bug）

- **现象**：git-bash / WSL 会话的 xterm 原始视图里键入 `ls` 完全不可见，但回车后输出正常。
- **排查**：用 `tmp/repro-echo.cjs`（node-pty 对照实验，复刻生产启动参数）跑
  「带注入 vs 裸启动」两组——注入组回显消失、对照组正常，锁定 `OSC133_BASH_INTEGRATION`。
- **根因**：`exec {__nav133_fd}>&1 2>/dev/null` 里的 `2>/dev/null` 是 **exec 自身的重定向**，
  对整个会话**持久生效**，shell 的 stderr 被永久丢进 /dev/null。
  而 bash 的交互提示符与键入回显都走 **stderr**——于是输入不可见；
  同一 bug 还会吞掉**所有命令的 stderr 输出**。
- **修复**：改为 `[ -n "${__nav133_fd:-}" ] || { exec {__nav133_fd}>&1; } 2>/dev/null`——
  组内静默 exec 可能的报错，不污染会话 stderr。实测：回显恢复，OSC 133 C/D 标记仍正常。
- **教训（改注入脚本时牢记）**：`exec` 的 IO 重定向对整个会话生效；
  给 exec 行加任何重定向都要先想清楚它会不会持续污染会话 fd。
  PowerShell / cmd 会话不受影响（无此注入）。源码内已加注释防回归。

### 10.2 GUI 视图块头滚动吸顶（commit `09ccae51b`，体验优化）

- **需求**：`cat` 长文档的块向下滚动时，块头（折叠按钮等）保持在块内顶部，
  随时可折叠，方便滚回更早的块。
- **实现**：`.command-block__header { position: sticky; top: 0 }` + 不透明底色（与块底色同色）。
