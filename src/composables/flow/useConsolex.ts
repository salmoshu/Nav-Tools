 ...
 */
export function useConsolex(useGlobal: boolean = true): ConsoleState {
  ...
  const handleRawDataBatch = (rawData: string) => {
    ...
    console.log(`批量导入完成，共导入 ${messages.value.length} 条消息`);
  };
  ...
  const connect = () => {
    ...
    addMessage("控制台已连接", "json", true);
  };

  const disconnect = () => {
    isConnected.value = false;
    addMessage("控制台已断开连接", "json", true);
  };
  ...
  const saveToFile = () => {
    ...
    try {
      ...
      console.log(`已保存 ${messages.value.length} 条消息到文件`);
    } catch (error) {
  ...
  watch(messages, (newMessages) => {
    console.log(`消息数量变化: ${newMessages.length}`);
  });

  watch(isPaused, (paused) => {
    console.log(`控制台${paused ? "暂停" : "恢复"}`);
  });

  // 初始化
  console.log("useConsolex 初始化完成");
