...
ipcRenderer.on('serial-data-to-renderer', (_, data:string) => {
...
export function useDevice() {
  ...
  const searchSerialPorts = () => {
    ipcRenderer.invoke('search-serial-ports').then((ports) => {
  ...
  const handleSerialSubmit = (): string => {
    ...
    const match = friendlyName.match(/\b([A-Z]+\d+(?:[A-Z]*\d*)*)\b(?=->|$|\))/i);
    const port = match ? match[1] : '';

    ipcRenderer.invoke('open-serial-port', {
        path: port,
