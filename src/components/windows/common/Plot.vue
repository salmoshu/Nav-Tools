...
{"time": 0.10, "camera_distance": 1.15, "camera_angle": 0.3, "pid_left_speed": 0.31, "pid_right_speed": 0.30, "motor_left_speed": 0.30, "motor_right_speed": 0.29}
  ...
  <PlotConfigDialog
    ...
    v-model:y-axis-config="yAxisConfig"
    :available-sources="availableSources"
    :single-chart-config="singleChartConfig"
    :single-chart-double-y-config="singleChartDoubleYConfig"
    :double-chart-config="doubleChartConfig"
    :double-chart-double-y-config="doubleChartDoubleYConfig"
    :validate-and-apply-config="validateAndApplyConfig"
    :export-config-file="exportConfigFile"
    @apply="applyViewConfig(createChart)"
  />
</template>
...
function updateChart() {
  ...
  chart.setOption(applyChartTheme(option))
}

function applyChartTheme(option: any) {
  ...
  axes.forEach(axis => {
    axis.axisLabel = { ...axis.axisLabel, color: colors.textMuted }
    axis.axisLine = {
      ...axis.axisLine,
      lineStyle: { ...axis.axisLine?.lineStyle, color: colors.border },
    }
    axis.splitLine = {
      ...axis.splitLine,
      lineStyle: { ...axis.splitLine?.lineStyle, color: colors.grid },
    }
  })
  ...
  return {
    ...
    textStyle: { ...option.textStyle, color: colors.text },
    title: option.title ? {
      ...option.title,
      textStyle: { ...option.title.textStyle, color: colors.text },
    } : option.title,
    legend: option.legend ? {
      ...option.legend,
      textStyle: { ...option.legend.textStyle, color: colors.text },
    } : option.legend,
    tooltip: option.tooltip ? {
      ...option.tooltip,
      backgroundColor: colors.surface,
      borderColor: colors.border,
      textStyle: { ...option.tooltip.textStyle, color: colors.text },
    } : option.tooltip,
  }
}
...
watch(resolvedTheme, () => createChart())
</script>
